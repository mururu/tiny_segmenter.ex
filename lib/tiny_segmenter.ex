defmodule TinySegmenter.Attributes do
...
defmodule TinySegmenter do
  use TinySegmenter.Attributes

  def segment("") do
    []
  end

  def segment(string) when is_binary(string) do
  ...
  def sum_scores(p1, p2, p3, [w1, w2, w3, w4, w5, w6], [c1, c2, c3, c4, c5, c6]) do
    ...
    Enum.reduce list, 0, fn([category|pattern_list], score)->
      pattern = Enum.join pattern_list
      score + (get_score(@score_map[category], pattern) || 0)
    end
  end
  ...
  def get_score(list, pattern) do
    nillify :proplists.get_value(pattern, list)
  end

  def nillify(value) do
    case value do
      :undefined -> nil
      v -> v
    end
